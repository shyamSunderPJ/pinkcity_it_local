...
function update_price_history_table(frm) {
	...
	for (let i = 0; i < frm.doc.price_history.length; i++) {
		if (frm.doc.price == frm.doc.price_history[i].price && frm.doc.weight_per_pcs_cts == frm.doc.price_history[i].weight_per_pcs_cts && frm.doc.btn_price == frm.doc.price_history[i].btn_price) {
			check = 1;
		}
	}
	if (check === 0) {
